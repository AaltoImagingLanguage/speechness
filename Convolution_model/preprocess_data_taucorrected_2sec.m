function [ dataC ] = preprocess_data_taucorrected( data, features, tau )
...
%      
    ...
    dataCausal_p = zeros(n_timePts, (tau+1)*n_voxels);
    iter1 = 0;
    for j = 1:n_timePts
      iter1 = iter1+1;
      for k = 1:n_voxels
	  ...
	  %[ R(1,t), R(1, t+1)...R(1,t+tau)... R(n,t), R(n,t+1).... R(n, t+tau)]
	  %where t corresponds to time points (j here)
	  iter2 = (k-1)*(tau+1) + 1;
	  if (j + tau + 1) <= size(dataCausal,2)
	    X = dataCausal(k,[j:j+tau]);
	    dataCausal_p(iter1, [iter2:(iter2+tau)]) = X;
	  else
	    X = dataCausal(k,[j:size(dataCausal,2)]);
	    Y = zeros(1,(tau+1)-length(X));
	    dataCausal_p(iter1, [iter2:(iter2+tau)]) = [X,Y];
	  end %end if
      end %end voxels
    end %end timePts
