...
tic
[ brainDataP ] = preprocess_data_taucorrected_2sec(brainDataS, spectrogramS, tau);
toc
size(brainDataP)
%size(spectrogramS)
clear brainDataS
...
%          spectrogram_S{iter1} = spectrogramS{i};
        spectrogram_S{iter1} = sum(spectrogramS{i}')';
        iter1 = iter1+1;
...
JOB_PATH = pwd
[ accuracy ] = crossValidateLeave2Out_featureScore_ampEnv([brainData_S],[spectrogram_S],1,100,JOB_PATH);
