function [ accuracy ] = crossValidateLeave2Out_featureScore_ampEnv( brainData, spectrogram, start_idx, end_idx, JOB_PATH )
...
%    fprintf('\njob: %d / %d',i,totalJobs);
    ...
    [trainingExamples, trainingTargets, testBrainData1, testBrainData2, testSpect1, testSpect2, mu_targets, sigma_targets,mu_examp, sigma_examp ] = ...
 ...
 %   tic
    ...
    testSpect1 = myzscore_reversed_witharguments(testSpect1, mu_targets, sigma_targets); %test is original spectrogram
    targetSpect1 = myzscore_reversed_witharguments(targetSpect1, mu_targets, sigma_targets); %predicted spectrogram
    testSpect2 = myzscore_reversed_witharguments(testSpect2, mu_targets, sigma_targets);
    targetSpect2 = myzscore_reversed_witharguments(targetSpect2, mu_targets, sigma_targets);
...
save([num2str(start_idx) '.mat'], 'targets','predictions', 'accuracy', 'result');
...
function [trainingExamples, trainingTargets, testBrainData1, testBrainData2, testSpect1, testSpect2, mu_targets, sigma_targets,mu_examp, sigma_examp ] = ...
...
%%% z-score only the training examples %%%
[trainingExamples, mu_examp, sigma_examp] = myzscore_extended(trainingExamples); %EXAMPLE = MEG
[trainingTargets, mu_targets, sigma_targets] = myzscore_extended(trainingTargets); %TARGETS = STIMULI FEATURES

%%% use the z-score statitic on test examples %%%
[testBrainData1] = myzscore_extended_witharguments(testBrainData1, mu_examp, sigma_examp); %EXAMPLE = MEG
[testSpect1] = myzscore_extended_witharguments(testSpect1, mu_targets, sigma_targets);
[testBrainData2] = myzscore_extended_witharguments(testBrainData2, mu_examp, sigma_examp); %EXAMPLE = MEG
[testSpect2] = myzscore_extended_witharguments(testSpect2, mu_targets, sigma_targets);
%%% end of z-score %%%
