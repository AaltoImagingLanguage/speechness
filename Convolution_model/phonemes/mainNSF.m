...
load('../../../../../data/speech_phonetic_model/phonetic_model_3.mat');
data = load('../../../../../data/semspeech_model_s03.mat'); %Finalized sensor level data - notice diff. subj. have diff. dates
clear mat;
...
for i = 1:length(phonetic_model)
    phonetic_model{i} = (phonetic_model{i})';
end

for i = 1:44
sizS = size(spectrogram{i+52},1);
sizP = size(phonetic_model{i},1);
  if sizS > sizP
    spectrogram{i+52}((sizP+1):end,:) = [];
  elseif sizS < sizP
    temp = zeros(length((sizS+1):sizP), size(spectrogram{i+52},2));
    spectrogram{i+52}((sizS+1):sizP,:) = temp;
  end
end
...
%          spectrogram_S{iter1} = spectrogramS{i};
	spectrogram_S{iter1} = phonetic_model{iter1};
        iter1 = iter1+1;
